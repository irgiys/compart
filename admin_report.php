...
$query = "SELECT p.id, p.name, p.desc, p.merk, p.picture, p.price, p.discount,p.created_at, p.modified_at ,p.deleted_at, p.category, pi.quantity,pi.sold, pi.id AS inventory_id
            FROM product AS p
            JOIN product_inventory AS pi ON (p.inventory_id = pi.id)  ORDER BY p.modified_at DESC";
$result = mysqli_query($conn, $query);
$products = [];
while ($product = mysqli_fetch_assoc($result)) {
    $products[] = $product;
}
$queryUser = "SELECT id,fullname,username,email,created_at FROM user";
$users = [];
$result = mysqli_query($conn, $queryUser);
while ($user = mysqli_fetch_assoc($result)) {
    $users[] = $user;
}
$querySeller = "SELECT id,fullname,username,email,created_at FROM seller";
$sellers = [];
$result = mysqli_query($conn, $querySeller);
...
<body class="min-vh-100">
    ...
    <div class="container-fluid px-md-5 mb-3">
        <ul class="nav nav-pills py-3">
            ...
            <li class="nav-item">
                <a class="nav-link active" href="admin_report.php">Report</a>
            </li>
            <li class="nav-item">
